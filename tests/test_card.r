...
test_retrieve <- function(api_key, customer_id) {
  ...
  card_id <- res$id
  stripe_retrieve_card(api_key = api_key, card_id, customer_id)
}
test_delete <- function(api_key, customer_id) {
  ...
  card_id <- res$id
  stripe_delete_card(
    api_key = api_key, customer_id, card_id)
}
...
test_that("retrieve card works", {
  expect_that(test_retrieve(
    api_key = api_key, customer_with_card)$object,
    matches("card"))
})
test_that("update card works", {
  expect_that(
    stripe_update_card(
      api_key = api_key, customer_with_card, card_id,
      list(name =
             paste(now))
    )$name,
    matches(paste(now))
...
test_that("list cards works", {
  expect_that(
    stripe_list_cards(api_key = api_key,
                      customer_with_card)$data$object,
    matches("card")
  )
})

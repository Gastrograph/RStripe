...
#'    further describe.}
#'    \item{statement_description}{\emph{optional} 15 character description
#'    to appear on
#'                                 statement.}
...
#' list(source="tok_14SsjM2eZvKYlo2CCtdx2AvO", amount=3000, currency="usd",
...
#' @export
#'
stripe_create_charge <- function(args, api_key = NULL) {
...
#' @export
#'
stripe_retrieve_charge <- function(charge_id, api_key = NULL) {
...
#' @export
#'
stripe_update_charge <- function(charge_id, args, api_key = NULL) {
...
#' Capture a charge.
#'
#' As part of the two step charge workflow, capture a charge.  Read more about
#' the workflow
...
#' \describe{
#'    \item{application_fee}{\emph{optional} A fee to add on to a charge.
#'    Only Stripe Connect}
...
#' List all charges.
#'
#' List all charges conducted in a time frame or on customer.  Limits to
#' 10 originally.
#'
#'
#' @param args an optional list that can include \strong{customer},
#' \describe{
#'    \item{customer}{\emph{optional} A customer id to show only their
#'    charges. }
#'    \item{ending_before}{\emph{optional} An id to show the ones before this
#'    item.}
#'    \item{limit}{\emph{optional} The limit to show.  Can be 1 to 100.
#'    Default is 10.}
#'    \item{starting_after}{\emph{optional} An id to show the ones starting
#'    after this item.}
#' }
#' @param api_key Your Stripe API Key
#'
#' @return Returns a data frame with all customer charges.  10 if no limit
#' specified.
#'
#' @export
#'
stripe_list_charges <- function(args = NULL, api_key = NULL) {

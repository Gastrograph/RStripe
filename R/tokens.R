...
#' Create a Card Token
#'
#' Create a card token to be used by other functions.  You can use this
#' to create a charge,
...
#' @export
#'
stripe_create_card_token <- function(args, api_key = NULL) {
  args <- .card(args)
  ...
  response <- POST(
    url = link,
    config = add_headers(Authorization = paste0("Bearer  ",
                                                api_key)),
    body = args,
    encode = "form"
  )
...
#' \describe{
#'    \item{country}{\strong{required} Country bank account is in.
#'    Only 'US' supported.}
#'    \item{routing_number}{\strong{required} routing number in string form.
#'    Not wire number.}
#'    \item{account_number}{\strong{required} Account number for account.
#'    Must be checking.}
#' }
#'
#' @examples \dontrun{
#' stripe_create_account_token(
#' api_key = "sk_test_BQokikJOvBiI2HlWgH4olfQ2",
#' list(country="US",
...
#' @export
#'
stripe_create_account_token <- function(args, api_key = NULL) {
